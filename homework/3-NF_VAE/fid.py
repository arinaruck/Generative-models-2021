...
@torch.no_grad()
def calculate_activation_statistics(dataloader, model, classifier, device):
    classifier.eval()
    upsample = nn.UpsamplingNearest2d(size=(128, 128)).to(device)
    ...
    for i, (image, label) in enumerate(dataloader):
        ...
        output_img = model.sample(b_sz)
        input_act = classifier(upsample(input_img))
        output_act = classifier(upsample(output_img))
...
@torch.no_grad()
def calculate_fid(dataloader, model, classifier, device):
    m1, s1, m2, s2 = calculate_activation_statistics(dataloader, model, classifier, device)
